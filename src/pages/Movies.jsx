...
const Movies = ({ standalone = false }) => {
  ...
  useEffect(() => {
    // If this is the standalone /movies page,
    // fetch immediately because the user intentionally opened it.
    if (standalone) {
      fetchMovies();
      return;
    }
  ...
  return (
    <section
      ref={sectionRef}
      className="relative min-h-[400px] overflow-hidden bg-[#09090B] px-4 pb-16 sm:px-6 lg:px-10 lg:pb-24"
    >
      {/* Background glow */}
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        {!loading && movies.length > 0 && (
          <div className="flex gap-4 overflow-x-auto pb-3 sm:gap-5 lg:grid lg:grid-cols-6 lg:overflow-visible [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {(standalone ? movies : movies.slice(0, 6)).map(
              (movie) => (
                <MovieCard
                  key={movie._id}
                  movie={movie}
